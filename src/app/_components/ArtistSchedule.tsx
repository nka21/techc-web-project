...
const ScheduleItem: React.FC<ScheduleItemProps> = ({ name, startTime, endTime, image }) => (
  <div className="flex items-center border-primary border-b-2 py-8">
    ...
    <div className="flex-1 flex flex-col justify-between h-56 py-12">
      <h5 className="font-semibold text-primary text-4xl tracking-[7.20px]">{name}</h5>
      <p className="text-3xl text-primary tracking-[3px]">{startTime} - {endTime}</p>
    </div>
  </div>
);
...
const ScheduleSection: React.FC<ScheduleSectionProps> = ({ title, timeRange, items }) => (
  <div className="my-12">
...
const ArtistSchedule: React.FC = () => {
  const morningSchedule: ScheduleItemProps[] = [
    { name: "SUPER BEAVER", startTime: "11:00", endTime: "11:45", image: superBeaver },
    { name: "THE ORAL CIGARETTES", startTime: "11:45", endTime: "12:30", image: oralCigarettes },
    { name: "COLD RAIN", startTime: "12:30", endTime: "13:15", image: coldRain },
  ];

  const afternoonSchedule: ScheduleItemProps[] = [
    { name: "saucy dog", startTime: "15:00", endTime: "15:45", image: saucyDog },
    { name: "HEY-SMITY", startTime: "15:45", endTime: "16:30", image: heySmity },
